...
/**
 * <p>Custom transparent overlay for gesture</p>
 * CustomGestureOverlayView is a custom transparent overlay for gesture input that can be placed on top of other
...
public class CustomGestureOverlayView extends FrameLayout {
    ...
    public CustomGestureOverlayView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        mGestureStrokeWidth = (float) 12.0;
        mInvalidateExtraBorder = Math.max(1, ((int) mGestureStrokeWidth) - 1);
        mCertainGestureColor = Color.BLUE;
        mUncertainGestureColor = Color.alpha(Color.BLUE);
        mFadeDuration = 150;
        mFadeOffset = 420;
        mGestureStrokeType = GESTURE_STROKE_TYPE_MULTIPLE;
        mGestureStrokeLengthThreshold = (float) 50.0;
        mGestureStrokeAngleThreshold = (float) 50.0;
        mGestureStrokeSquarenessTreshold = (float) 0.275;
    ...
    /**
     * Only pen event call processEvent() for detecting Gesture.
     */
    @Override
    public boolean dispatchTouchEvent(MotionEvent event) {
    	
        SPenEvent sPenEvent;
        sPenEvent = SPenLibrary.getEvent(event);

        // Only pen event call processEvent() for detecting Gesture.
        if (isEnabled() && sPenEvent.isPen()) {
    ...
    private void fireOnGesturePerformed() {
        ...
        for (int i = 0; i < count; i++) {
            actionListeners.get(i).onGesturePerformed(CustomGestureOverlayView.this, mCurrentGesture);
        }
    }
    ...
    /**
     * Interface for OnGesturingListener
     *
     */
    public static interface OnGesturingListener {
        void onGesturingStarted(CustomGestureOverlayView overlay);

        void onGesturingEnded(CustomGestureOverlayView overlay);
    }

    /**
     * Interface for OnGestureListener
     *
     */
    public static interface OnGestureListener {
        void onGestureStarted(CustomGestureOverlayView overlay, MotionEvent event);

        void onGesture(CustomGestureOverlayView overlay, MotionEvent event);

        void onGestureEnded(CustomGestureOverlayView overlay, MotionEvent event);

        void onGestureCancelled(CustomGestureOverlayView overlay, MotionEvent event);
    }

    /**
     * Interface for OnGesturePerformedListener
     *
     */
    public static interface OnGesturePerformedListener {
        void onGesturePerformed(CustomGestureOverlayView overlay, Gesture gesture);
    }
}
